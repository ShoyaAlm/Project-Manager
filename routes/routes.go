...
func SetChecklistRoutes(r *mux.Router) {
	...
	r.HandleFunc("/api/lists/{id:[0-9]+}/cards/{cardID:[0-9]+}/checklists/{checklistID:[0-9]+}/item-to-checklist-order", controllers.MoveItemToChecklist).Methods("PUT")
...
func SetSignInUpRoutes(r *mux.Router) {
	...
	// r.HandleFunc("/api/hash", controllers.HashAllPasses).Methods("PATCH")
	r.HandleFunc("/api/signup", controllers.SignUp).Methods("POST")
	r.HandleFunc("/api/login", controllers.Login).Methods("POST")

	r.HandleFunc("/api/users/{userID:[0-9]+}", controllers.UpdateUserInfo).Methods("PUT")
